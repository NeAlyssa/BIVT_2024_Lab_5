...
public class Program
{
    ...
    public void Task_2_2(double[] A, double[] B)
    {
        ...
        {
            for (int j = maxIndexA + 1; j < A.Length; j++)
            {
                sum += A[j];
                count += 1;
            }
            ...
            {
                if (A[j] == A[maxIndexA]) A[j] = average;
            }
        }
        else if (maxIndexA > maxIndexB)
        {
            for (int j = maxIndexB + 1; j < B.Length; j++)
            {
                sum += B[j];
                count += 1;
            }
            ...
            {
                if (B[j] == B[maxIndexB]) B[j] = average;
            }
        }
    ...
    // Метод для поиска индекса столбца с максимальным элементом
    ...
    // Метод для сортировки элементов строки в порядке возрастания
    private void SortRowAscending(int[,] matrix, int rowIndex)
    {
        ...
        {
            tempArray[colIndex] = matrix[rowIndex, colIndex];
        }
        Array.Sort(tempArray);

        for (int colIndex = 0; colIndex < columns; colIndex++)
        {
            matrix[rowIndex, colIndex] = tempArray[colIndex];
        }
    }
    ...
    // Метод для сортировки отрицательных элементов в массиве
    private void SortNegativeElements(int[] array)
    {
        ...
        negativeNumbers.Sort((x, y) => y.CompareTo(x));
    ...
    // Метод для сортировки элементов главной диагонали матрицы
    ...
    public void Task_2_20(ref int[,] A, ref int[,] B)
    {
        ...
        for (int col = A.GetLength(1) - 1; col >= 0; col--)
        {
            ...
            if (!hasZero) RemoveColumn(ref A, col);
        }

        for (int col = B.GetLength(1) - 1; col >= 0; col--)
        {
            bool hasZero = false;
            ...
            if (!hasZero) RemoveColumn(ref B, col);
        }
    }
